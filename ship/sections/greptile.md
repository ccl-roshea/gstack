...
- **72** — the reviewer POSTED, but no FINISHED review covers the head revision: a STALE review, not a silent one. Print the script's stderr verbatim — a `STALE REVIEW:` line lists the unreviewed files where a reviewed revision exists to compare from (VERSION, changelogs, READMEs and root prose are pre-excused, so whatever is listed is real; a reviewer with no revision binding gets the headline alone). Never report it as a pass, and never hard-stop the way 70 does: staleness is a judgement call, silence is not. If a finding says the reviewer `has not FINISHED reviewing the head revision`, it is reading head RIGHT NOW — waiting resolves it. **Ask** before shipping: A) Wait for the re-review, then re-run this check (recommended) B) Ship anyway — name the listed files as knowingly unreviewed in the ship summary.
- **any other code** — the checker itself failed: a crash (1), a bad invocation (2), a missing `python3` (127). Treat exactly as 71 — print the exit code and stderr, and report it as missing coverage, never as a clean bill. A silent fall-through here rebuilds the very gap this assertion exists to close, one exit code off-contract.
